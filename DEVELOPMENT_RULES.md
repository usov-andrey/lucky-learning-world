...
## 11. Approved Spelling Audio Voice

- **Required Voice**: All newly generated Lucky Learning World spelling word and definition audio MUST use the British English neural voice `en-GB-SoniaNeural` at rate `-15%`.
- **No Silent Fallback**: Never publish audio generated by an OS-default, locale-implicit, or otherwise unapproved fallback voice. Generation must fail if Sonia cannot be used.
- **Voice Change Gate**: Any proposed voice change requires a representative sample and explicit user approval before bulk generation, commit, or deployment.
- **Audio Provenance**: Each generated lesson audio directory must record the exact voice, rate, generator, and approval basis in a local provenance file.
